...
func Subscribe(c *gin.Context) {
	...
	if len(mail) > 0 {
		...
		if err == nil {
			if !subscriber.VerifyState && subscriber.OutTime != nil && helpers.GetCurrentTime().After(*subscriber.OutTime) { //激活链接超时
...
func sendActiveEmail(subscriber *models.Subscriber) (err error) {
	...
	outTime := helpers.GetCurrentTime().Add(duration)
	subscriber.OutTime = &outTime
...
func ActiveSubscriber(c *gin.Context) {
	...
	if subscriber.OutTime == nil || !helpers.GetCurrentTime().Before(*subscriber.OutTime) {
		HandleMessage(c, "激活链接已过期，请重新获取！")
		return
	}
	subscriber.VerifyState = true
	currentTime := helpers.GetCurrentTime()
	subscriber.OutTime = &currentTime
	err = subscriber.Update()
	if err != nil {
